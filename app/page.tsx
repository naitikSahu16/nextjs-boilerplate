...
export default function Home() {
  ...
  // Simulates the login process when they click Google/GitHub in the popup
  const handleSimulatedLogin = (provider) => {
    setShowAuthModal(false);
    setIsLoggedIn(true);
    setKey("tt_founder_999"); 
    alert(`✅ Successfully authenticated with ${provider}! (Demo Mode: Founder key auto-filled for testing)`);
  };
    ...
    <div className="min-h-screen bg-[#020614] text-white font-sans py-6 px-4 flex flex-col items-center selection:bg-[#00e5b5] selection:text-black overflow-x-hidden relative">
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-md bg-[#0b1221] border border-[#1e293b] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 md:p-8">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-white">Create your account</h3>
                <button onClick={() => setShowAuthModal(false)} className="text-slate-400 hover:text-white transition-colors">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
              </div>
              
              <div className="space-y-3">
                <button onClick={() => handleSimulatedLogin('Google')} className="w-full flex items-center justify-center gap-3 bg-white text-black font-semibold py-3 px-4 rounded-xl hover:bg-slate-100 transition-colors">
                ...
                <button onClick={() => handleSimulatedLogin('GitHub')} className="w-full flex items-center justify-center gap-3 bg-[#24292e] text-white font-semibold py-3 px-4 rounded-xl hover:bg-[#2c3238] transition-colors border border-[#30363d]">
              ...
              <p className="mt-5 text-center text-xs text-slate-500">
                By clicking continue, you agree to our <span className="text-[#00e5b5] cursor-pointer">Terms of Service</span> and <span className="text-[#00e5b5] cursor-pointer">Privacy Policy</span>.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-[800px] flex flex-col gap-6">
        ...
        <nav className="flex items-center justify-between w-full pb-4">
          ...
          <div className="flex items-center gap-4">
            {!isLoggedIn ? (
              <>
                <button onClick={handleAuthClick} className="text-sm font-medium text-slate-400 hover:text-white transition-colors hidden sm:block">
                  Sign in
                </button>
                <button onClick={handleAuthClick} className="bg-[#00e5b5] text-black text-sm font-bold py-1.5 px-4 rounded-lg hover:bg-[#00c090] transition-all shadow-[0_0_15px_rgba(0,229,181,0.15)]">
                  Get API Key
                </button>
              </>
            ) : (
